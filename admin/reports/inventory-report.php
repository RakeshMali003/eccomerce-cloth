...
foreach ($inventory as &$item) {
    ...
    $item['cost_price'] = $item['last_cost'] ?: ($item['price'] * 0.7); // Fallback to 70% of retail if no cost history
            ...
            <tbody class="divide-y divide-slate-50">
                <?php foreach ($inventory as $item):
                    ...
                    <tr class="hover:bg-slate-50 transition-all">
                        ...
                        <td class="px-6 py-5 text-right text-xs font-bold text-slate-400">₹
                            <?= number_format($item['cost_price'], 2) ?>
                        </td>
                        <td class="px-6 py-5 text-right text-xs font-bold text-slate-900">₹
                            <?= number_format($item['price'] ?? 0, 2) ?>
                        </td>
                        <td class="px-8 py-5 text-right font-black text-slate-900">₹
                            <?= number_format($item['total_stock'] * $item['cost_price'], 2) ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
