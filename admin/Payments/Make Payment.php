...
$suppliers = $pdo->query("
    SELECT s.supplier_id, s.name, SUM(b.balance_amount) as total_debt 
    FROM suppliers s
...
<div class="mb-10">
        <h2 class="text-4xl font-black tracking-tighter text-slate-900">Financial Pulse<span class="text-orange-600">.</span></h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
        ...
        <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm border-b-4 border-b-orange-500">
            ...
            <h3 class="text-3xl font-black text-orange-600"><?= $pendingCount ?> <span class="text-sm">Invoices</span></h3>
        </div>
        ...
        <div class="bg-slate-900 p-8 rounded-[2.5rem] shadow-2xl">
            ...
            <h3 class="text-3xl font-black text-white"><?= str_pad($upcomingDues, 2, '0', STR_PAD_LEFT) ?> <span class="text-sm text-orange-500">Alerts</span></h3>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm">
            ...
            <div class="space-y-6">
                ...
                <div class="flex items-center justify-between p-4 rounded-2xl bg-slate-50/50 border border-transparent hover:border-slate-100 transition-all">
                    <div class="flex items-center gap-4">
                        <div class="w-10 h-10 rounded-xl <?= $act['type'] == 'bill' ? 'bg-orange-100 text-orange-600' : 'bg-emerald-100 text-emerald-600' ?> flex items-center justify-center">
                            <i class="fas <?= $act['type'] == 'bill' ? 'fa-file-invoice-dollar' : 'fa-check-double' ?> text-xs"></i>
                        </div>
                        <div>
                            <p class="text-xs font-black text-slate-900"><?= $act['msg'] ?></p>
                            <p class="text-[9px] font-bold text-slate-400 uppercase"><?= date('d M, H:i', strtotime($act['date'])) ?></p>
                        </div>
                    </div>
        ...
        <div class="bg-slate-900 rounded-[3rem] p-10 shadow-2xl text-white">
            ...
            <div class="grid grid-cols-1 gap-4">
                <a href="Purchases/Purchase Bill.php" class="flex items-center justify-between p-5 bg-white/5 rounded-2xl hover:bg-orange-600 transition-all group">
                    <span class="text-[10px] font-black uppercase tracking-widest">New Purchase Bill</span>
                    <i class="fas fa-arrow-right text-xs group-hover:translate-x-1 transition-transform"></i>
                </a>
                <a href="Payments/Make Payment.php" class="flex items-center justify-between p-5 bg-white/5 rounded-2xl hover:bg-emerald-600 transition-all group">
                    <span class="text-[10px] font-black uppercase tracking-widest">Record Payment</span>
                    <i class="fas fa-arrow-right text-xs group-hover:translate-x-1 transition-transform"></i>
                </a>
                <a href="reports/Pending Bills.php" class="flex items-center justify-between p-5 bg-white/5 rounded-2xl hover:bg-blue-600 transition-all group">
        ...
        <form action="process_payment.php" method="POST" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="relative">
                    ...
                    <div id="search_results" class="absolute z-50 w-full bg-white shadow-2xl rounded-2xl mt-2 max-h-60 overflow-y-auto hidden border border-slate-100">
                        <?php foreach($suppliers as $s): ?>
                            <div class="p-4 hover:bg-slate-50 cursor-pointer border-b border-slate-50 supplier-opt" 
                                ...
                                <p class="text-[9px] text-red-500 font-bold uppercase tracking-widest">Owed: ₹<?= number_format($s['total_debt'], 2) ?></p>
                            </div>
                        <?php endforeach; ?>
                    </div>
                ...
                <div>
                    ...
                    <select name="bill_id" id="pay_bill" required class="w-full bg-slate-50 p-4 rounded-2xl font-bold outline-none appearance-none">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="space-y-1">
                    ...
                    <input type="number" name="amount" placeholder="₹ 0.00" required class="w-full bg-slate-50 p-4 rounded-2xl font-bold outline-none">
                </div>
                
                <div class="space-y-1">
                    <label class="text-[10px] font-black uppercase text-slate-400 ml-2">Mode</label>
                    <select name="payment_mode" onchange="toggleRefFields(this.value)" class="w-full bg-slate-50 p-4 rounded-2xl font-bold outline-none">
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" name="transaction_id" id="ref_field" placeholder="ID / Cheque #" class="w-full bg-slate-50 p-4 rounded-2xl font-bold outline-none">
                </div>
            </div>

            <button type="submit" class="w-full bg-slate-900 text-white py-6 rounded-[2rem] font-black uppercase tracking-widest hover:bg-emerald-600 transition-all shadow-xl shadow-slate-200">
                Synchronize Payment
            </button>
        </form>
    </div>
