...
?>
    ...
    <style>
        ...
        .slide-over { transition: transform 0.4s cubic-bezier(0.4, 0, 0.2, 1); }
    </style>
...
<main class="p-6 lg:p-12">
    ...
    <header class="sticky top-0 z-30 glass-panel border-b border-slate-100 px-8 py-4 flex justify-between items-center rounded-3xl mb-8">
        <div>
        ...
        <button onclick="openForm('add')" class="bg-slate-900 text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-orange-600 transition-all shadow-lg shadow-slate-200">
            + New Vendor
        </button>
    </header>

    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <div class="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm border-b-4 border-b-orange-500 hover:shadow-md transition-all">
        ...
        <div class="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm border-b-4 border-b-red-500 hover:shadow-md transition-all">
        ...
        <div class="bg-slate-900 p-8 rounded-[2rem] shadow-xl hover:-translate-y-1 transition-all">
            ...
            <h3 class="text-3xl font-black text-orange-500"><?php echo str_pad($due_this_week, 2, '0', STR_PAD_LEFT); ?> Bills</h3>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <?php else: ?>
                        ...
                        <tr class="group hover:bg-slate-50/30 transition-all cursor-pointer" onclick='fetchAndShowView(<?php echo $s["supplier_id"]; ?>)'>
                            <td class="px-8 py-5">
                                <p class="text-sm font-black text-slate-900"><?php echo htmlspecialchars($s['name']); ?></p>
                                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">GST: <?php echo $s['gstin'] ?: 'Not Provided'; ?></p>
                            </td>
                            ...
                            <td class="px-6 py-5">
                                <span class="text-[10px] font-black text-slate-500 bg-slate-100 px-3 py-1 rounded-lg uppercase">
                                    <?php echo $s['payment_terms'] ?: 'COD'; ?>
                                </span>
                            </td>
                            <td class="px-6 py-5">
                                <span class="px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest <?php echo ($s['status'] == 'active') ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'; ?>">
                                    <?php echo $s['status']; ?>
                                </span>
                            </td>
                            <td class="px-8 py-5 text-right">
                                <div class="flex justify-end gap-2" onclick="event.stopPropagation()">
                                    <button onclick='fetchAndShowEdit(<?php echo $s["supplier_id"]; ?>)' class="w-9 h-9 flex items-center justify-center bg-slate-50 text-slate-400 rounded-xl hover:bg-slate-900 hover:text-white transition-all">
                                        <i class="fas fa-pencil text-xs"></i>
                                    </button>
                                    <button onclick="confirmDeletion(<?php echo $s['supplier_id']; ?>)" class="w-9 h-9 flex items-center justify-center bg-slate-50 text-slate-400 rounded-xl hover:bg-red-500 hover:text-white transition-all">
    ...
    <div id="sideDrawer" class="fixed inset-0 z-[100] hidden">
        ...
        <div class="absolute top-0 right-0 h-full w-full max-w-xl bg-white shadow-2xl slide-over translate-x-full flex flex-col">
            <div class="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <h3 id="drawerTitle" class="text-2xl font-black text-slate-900">Add Supplier</h3>
                <button onclick="closeDrawer()" class="text-slate-400 hover:text-red-500"><i class="fas fa-times text-xl"></i></button>
            </div>
            ...
            <form action="process_supplier.php" method="POST" class="p-8 space-y-6 overflow-y-auto custom-scroll flex-1">
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" name="name" id="in_name" required class="w-full bg-slate-50 p-4 rounded-2xl border-2 border-transparent focus:border-orange-500 outline-none text-sm font-bold">
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1">
                        <label class="text-[10px] font-black uppercase text-slate-400 ml-2">Contact Person</label>
                        <input type="text" name="contact_person" id="in_contact" class="w-full bg-slate-50 p-4 rounded-2xl border-2 border-transparent focus:border-orange-500 outline-none text-sm font-bold">
                    </div>
                    <div class="space-y-1">
                        <label class="text-[10px] font-black uppercase text-slate-400 ml-2">Phone *</label>
                        <input type="text" name="phone" id="in_phone" required class="w-full bg-slate-50 p-4 rounded-2xl border-2 border-transparent focus:border-orange-500 outline-none text-sm font-bold">
                    </div>
                </div>
                <div class="space-y-1">
                    <label class="text-[10px] font-black uppercase text-slate-400 ml-2">Official Email</label>
                    <input type="email" name="email" id="in_email" class="w-full bg-slate-50 p-4 rounded-2xl border-2 border-transparent focus:border-orange-500 outline-none text-sm font-bold">
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1">
                        <label class="text-[10px] font-black uppercase text-slate-400 ml-2">GSTIN</label>
                        <input type="text" name="gstin" id="in_gst" class="w-full bg-slate-50 p-4 rounded-2xl border-2 border-transparent focus:border-orange-500 outline-none text-sm font-bold">
                    </div>
                    <div class="space-y-1">
                        <label class="text-[10px] font-black uppercase text-slate-400 ml-2">Status</label>
                        <select name="status" id="in_status" class="w-full bg-slate-50 p-4 rounded-2xl outline-none text-sm font-bold">
                ...
                <div class="space-y-1">
                    ...
                    <textarea name="address" id="in_addr" rows="2" class="w-full bg-slate-50 p-4 rounded-2xl border-2 border-transparent focus:border-orange-500 outline-none text-sm font-bold resize-none"></textarea>
                </div>

                <div class="flex gap-4 pt-10">
                    <button type="submit" class="flex-1 bg-slate-900 text-white py-5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-xl hover:bg-orange-600 transition-all">Synchronize Vendor</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="bg-white w-full max-w-4xl rounded-[3rem] shadow-2xl relative p-10 overflow-y-auto max-h-[90vh]">
        <div class="flex justify-between items-start mb-10">
            ...
            <button onclick="closeView()" class="w-12 h-12 bg-slate-50 rounded-full flex items-center justify-center text-slate-400 hover:text-red-500 transition-all"><i class="fas fa-times"></i></button>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div class="md:col-span-1 space-y-6">
                ...
                <div class="space-y-4 px-4">
                    <div><p class="text-[9px] font-black text-slate-400 uppercase">Email</p><p id="v_email" class="text-xs font-bold text-slate-800"></p></div>
                    <div><p class="text-[9px] font-black text-slate-400 uppercase">Phone</p><p id="v_phone" class="text-xs font-bold text-slate-800"></p></div>
                    <div><p class="text-[9px] font-black text-slate-400 uppercase">Address</p><p id="v_address" class="text-xs font-medium text-slate-500 leading-relaxed"></p></div>
                </div>
            </div>

            <div class="md:col-span-2">
                <div class="flex gap-4 border-b border-slate-100 mb-6">
                    <button onclick="switchTab('history')" id="tab-history-btn" class="view-tab-btn pb-3 text-[10px] font-black uppercase tracking-widest border-b-2 border-orange-500 text-slate-900">Purchase History</button>
                    <button onclick="switchTab('ledger')" id="tab-ledger-btn" class="view-tab-btn pb-3 text-[10px] font-black uppercase tracking-widest border-b-2 border-transparent text-slate-400">Bills & Ledger</button>
                </div>
