...
$stmt = $pdo->prepare("SELECT * FROM settings WHERE group_name = 'payment' ORDER BY setting_key");
$stmt->execute();
...
<main class="p-6 lg:p-12 bg-[#f8fafc] min-h-screen">
    <div class="mb-10 flex items-center gap-4">
        ...
        <div>
            <h2 class="text-3xl font-black tracking-tighter text-slate-900">Payment Gateway<span
                    class="text-emerald-600">.</span></h2>
            <p class="text-slate-400 text-sm font-medium">Configure Stripe, Razorpay, and Cash on Delivery parameters.
            </p>
        </div>
    </div>
            ...
            <?php foreach ($settings as $s): ?>
                <div class="space-y-2">
                    ...
                    <input type="text" name="settings[<?= $s['setting_key'] ?>]"
                        ...
                        class="w-full bg-slate-50 px-8 py-5 rounded-[2rem] text-sm font-black outline-none border-2 border-transparent focus:border-emerald-500 transition-all">
                </div>
            <?php endforeach; ?>
        </div>

        <button type="submit"
            class="w-full bg-slate-900 text-white py-8 rounded-[3rem] font-black uppercase tracking-[0.3em] shadow-2xl shadow-slate-200 hover:bg-emerald-600 transition-all group">
            Secure Payment Config <i class="fas fa-shield-alt ml-4 group-hover:rotate-12 transition-transform"></i>
        </button>
    </form>
</main>
